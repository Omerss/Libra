...
namespace LibraAlchemy
{
    class DAL
    {
        ...
        public static List<Product> Get_Products(List<int> prods_ids)
        {
            ...
            string id_list = IDListToString(prods_ids);
            cm.CommandText = "SELECT * FROM Products WHERE ID IN(" + id_list + ")";
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                ...
                //Get Procedure by Procedure_ID

                //Get Effects and make this product their Owner
        ...
        public static List<Special> Get_Specials(List<int> special_id_list)
        {
        ...
        public static Dictionary<Effect, Effect> GetSpecialEffectPairs(int Spec_ID)
        {
            ...
            if (pairs_ids.Any())
            {
                ...
                effect_pairs = keylist.Zip(vallist, (k, v) => new { k, v }).ToDictionary(x => x.k, x => x.v);
            }
            con.Close();
            return effect_pairs;
        }
