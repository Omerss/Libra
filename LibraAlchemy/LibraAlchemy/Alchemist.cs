    ...
    class Alchemist
    {
        ...
        public List<Potion> GetSimilarPotions(List<FinalEffect> effects)
        {
            ...
            if (effects.Count < 5)
            {
                ...
                test_pots = Potion_List.FindAll(item => item.Effects.Count == effects.Count + 1);
                foreach (Potion pot in test_pots)
                {
                    //now we will see if any of them contains effect_list (and has effects of the same level).
                    if (!effects.Except(pot.Effects, new EffectAndLevelComparer()).Any())
                    {
                        matches.Add(pot);
                    }
                }
            ...
            if (effects.Count > 1)
            {
                ...
                foreach (Potion pot in test_pots)
                {
                    ...
                    if (!pot.Effects.Except(effects, new EffectAndLevelComparer()).Any())
                    {
                        matches.Add(pot);
                    }
                }
            ...
            test_pots = Potion_List.FindAll(item => item.Effects.Count == effects.Count);
            ...
            for (int i = 0; i < effects.Count; i++)
            {
                ...
                foreach (Potion pot in test_pots)
                {
                    ...
                    if (!temp_list.Except(pot.Effects, new EffectAndLevelComparer()).Any())
                    {
                        ...
                        if (!effects.SequenceEqual(pot.Effects,new EffectAndLevelComparer()))
                            matches.Add(pot);
                    }
                }
            }
            ...
            test_pots = Potion_List.FindAll(item => item.Effects.SequenceEqual(effects,new EffectComparer()));
            //Remove all of the same (levelwise)
            test_pots.RemoveAll(item => item.Effects.SequenceEqual(effects, new EffectAndLevelComparer()));
